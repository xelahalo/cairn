...
impl Filesystem for TracerFS {
    ...
    fn setattr(
        ...
        let real_attrs = match fs::metadata(&attrs.real_path) {
            Ok(attrs) => attrs,
            Err(error) => {
                error!("Failed to get metadata for {:?}: {}", attrs.real_path, error);
        ...
        if let Some(mode) = mode {
            ...
            // change the permissions on the real file
            let mut real_permissions = real_attrs.permissions();
            real_permissions.set_mode(mode);
            fs::set_permissions(&attrs.real_path, real_permissions).unwrap();
        ...
        if uid.is_some() || gid.is_some() {
            ...
            // Change the mode and the ownership of the real file
            let mut real_permissions = real_attrs.permissions();
            real_permissions.set_mode(attrs.mode as u32);
            fs::set_permissions(&attrs.real_path, real_permissions).unwrap();
            ufs::chown(&attrs.real_path, uid, gid).unwrap();
